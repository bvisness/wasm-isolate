...
func _error_3(s *Stream, pos int, msg string) {
	panic(CodeError{
...
func _require_4(b bool, s *Stream, pos int, msg string) {
	if !b {
		_error_3(s, pos, msg)
	}
}
...
func _expect_3(b byte, s *Stream, msg string) {
	_require_4(_get(s) == b, s, _pos(s)-1, msg)
}

func _illegal_3(s *Stream, pos int, b byte) {
	_error_3(s, pos, "illegal opcode "+_string_of_byte(b))
}

func _illegal2_4(s *Stream, pos int, b byte, n int) {
